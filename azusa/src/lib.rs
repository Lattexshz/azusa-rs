...
impl Azusa {
    ...
    pub fn draw_point(&self, color: Color) {
        self.target.borrow_mut().push(DrawTarget::DrawPoint(
            *self.x.borrow_mut(),
            *self.y.borrow_mut(),
            color,
        ));
    ...
    pub fn draw_rectangle(&self, color: Color, width: u32, height: u32, thickness: u32) {
        self.target.borrow_mut().push(DrawTarget::DrawRectangle(
            *self.x.borrow_mut(),
            *self.y.borrow_mut(),
            width,
            height,
    ...
    pub fn fill_rectangle(&self, color: Color, width: u32, height: u32) {
        self.target.borrow_mut().push(DrawTarget::FillRectangle(
            *self.x.borrow_mut(),
            *self.y.borrow_mut(),
            width,
            height,
